  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold text-[#1d1d1f]">Curator Q&A (FAQs)</h1>
        <Link 
          href="/admin/faqs/new"
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-black/5 border-b border-black/5">
              <th className="px-6 py-4 font-semibold text-[0.9rem]">Question</th>
          ...
          <tbody>
            {faqs.map(faq => (
              <tr key={faq.id} className="border-b border-black/5 hover:bg-black/[0.02] transition-colors">
                <td className="px-6 py-4 font-medium max-w-md truncate">{faq.question}</td>
                <td className="px-6 py-4 text-[#86868b]">{faq.sort_order}</td>
                <td className="px-6 py-4">
                  <span className={`px-3 py-1 rounded-full text-xs font-bold ${faq.is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
            ...
            {faqs.length === 0 && (
              <tr>
                <td colSpan="4" className="px-6 py-12 text-center text-[#86868b]">No FAQs found.</td>
              </tr>
            )}
          </tbody>
        </table>
