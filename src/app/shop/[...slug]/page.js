  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="w-10 h-10 rounded-full border-4 border-[#B8860B]/20 border-t-[#B8860B] animate-spin"></div>
      </div>
    );
  }
        ...
        <div className="container">
          <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-16 lg:gap-24 items-start">
            ...
            <div className="flex flex-col gap-6 reveal sticky top-32">
              <div className="aspect-square bg-white overflow-hidden rounded-[2px] border border-black/5 shadow-2xl shadow-[#1F1F1F]/5 relative group">
                {currentImage ? (
                  <img src={currentImage} alt={product.name} className="w-full h-full object-contain p-12 transition-transform duration-[1.5s] group-hover:scale-105" />
                ) : (
              ...
              {images.length > 1 && (
                <div className="grid grid-cols-5 gap-4">
                  {images.map((img, index) => (
                    <button
                      key={img.id}
                      className={`aspect-square bg-white cursor-pointer rounded-[2px] overflow-hidden border transition-all duration-500 ${index === activeImage ? 'border-[#B8860B] ring-1 ring-[#B8860B] shadow-lg' : 'border-black/5 opacity-60 hover:opacity-100'}`}
                      onClick={() => setActiveImage(index)}
                    >
                      <img src={img.file_path} alt={img.alt_text || product.name} className="w-full h-full object-cover p-2" />
                    </button>
                  ))}
                </div>
              )}
