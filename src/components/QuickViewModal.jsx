...
import { ShoppingCart, X, Ruler, Weight, Package, ChevronLeft, ChevronRight, MessageSquare, ShieldCheck, Truck } from 'lucide-react';
      ...
      <div className="relative bg-white w-full max-w-[1100px] max-h-[90vh] overflow-y-auto rounded-sm shadow-2xl flex flex-col md:flex-row animate-in fade-in zoom-in duration-300">
        ...
        <div className="w-full md:w-1/2 flex flex-col bg-[#F9F9F9]">
          <div className="relative aspect-square w-full group">
            ...
            {images.length > 1 && (
              <>
                <button onClick={handlePrev} className="absolute left-4 top-1/2 -translate-y-1/2 p-2 bg-white/80 hover:bg-white rounded-full shadow-md opacity-0 group-hover:opacity-100 transition-all" suppressHydrationWarning>
                  <ChevronLeft className="w-5 h-5" />
                </button>
                <button onClick={handleNext} className="absolute right-4 top-1/2 -translate-y-1/2 p-2 bg-white/80 hover:bg-white rounded-full shadow-md opacity-0 group-hover:opacity-100 transition-all" suppressHydrationWarning>
        ...
        <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center">
          ...
          <h2 className="text-3xl md:text-4xl font-heading text-[var(--color-text-primary)] mb-4 leading-tight">
            {product.name}
          </h2>
            ...
            <div className="mb-6">
              {hasDiscount ? (
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-[1.8rem] font-heading text-[var(--color-accent)] italic">₹{salePrice.toLocaleString()}</span>
                </div>
              ) : price > 0 ? (
                <span className="text-[1.8rem] font-heading text-[var(--color-text-primary)] italic">₹{price.toLocaleString()}</span>
              ) : null}
            </div>
          )}

          <div 
            className="text-[var(--color-text-secondary)] text-[0.85rem] leading-relaxed mb-8 font-light artisan-description max-h-[150px] overflow-y-auto pr-2"
            dangerouslySetInnerHTML={{ __html: product.short_description || product.description || 'This exquisite masterpiece is handcrafted by our master artisans in Udaipur.' }}
          />

          {/* Technical Specs Mini-Grid */}
          {(product.weight || product.length || product.width || product.height) && (
            <div className="grid grid-cols-2 gap-3 mb-8 p-4 bg-[#F9F7F5] border border-black/[0.03] rounded-sm">
              {product.weight > 0 && (
                <div className="flex items-center gap-2">
                  <Weight className="w-3 h-3 text-[var(--color-accent)]" />
                  <span className="text-[0.65rem] font-medium text-[#1d1d1f]">{product.weight} kg</span>
                </div>
              )}
              {product.length > 0 && (
                <div className="flex items-center gap-2">
                  <Ruler className="w-3 h-3 text-[var(--color-accent)]" />
                  <span className="text-[0.65rem] font-medium text-[#1d1d1f]">{product.length}×{product.width}×{product.height} cm</span>
                </div>
              )}
            </div>
          )}
          ...
          <div className="flex flex-col gap-4">
            {is_ecommerce ? (
              <button
                ...
                className="w-full flex items-center justify-center gap-3 py-4 bg-black text-white text-[0.7rem] uppercase tracking-[0.2em] font-bold transition-all hover:bg-[var(--color-accent)] hover:shadow-lg"
                suppressHydrationWarning
              >
